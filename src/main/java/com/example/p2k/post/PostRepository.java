...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select p from Post p where p.course.id = :courseId and p.category = :category and (p.open = true or p.user.id = :userId)")
    Page<Post> findPostByCategory(Pageable pageable, @Param("courseId") Long courseId, @Param("category") Category category, @Param("userId") Long userId);
